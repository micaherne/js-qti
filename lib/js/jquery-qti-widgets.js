(function($, window){
	...
	QTI.Variable.prototype = $.extend({
		...
		setValue: function(value) {
			var self = this;
			if($.isArray(value)) {
				self._value = value.slice(); // copy the contents
			} else {
				self._value = [value];
			}
		},
	...
	$.widget("qti.assessmentItem", {
		...
		addVariable: function(variableDeclaration) {
			var self = this;
			var variable = $.extend(true, { _value: [] }, variableDeclaration);
			self.variables[variable.identifier] = variable;
			if(variable.qtiElement) {
	...
	$.widget("qti.endAttemptInteraction", {
		...
		_create: function() {
			...
			var button = $('<input type="button"/>').val(title).on('click', function(e){
				self.options.assessmentItem.setVariable(responseIdentifier, true);
