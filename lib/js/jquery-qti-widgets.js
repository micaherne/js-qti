(function($, window){
	 ...
	 * Common objects.
	 * 
	 * TODO: QTI.Variable doubles as a declaration and an instance. Bad?
	 */
	QTI.Variable = function(params) {
		if(this instanceof QTI.Variable){
			...
			if(params.nodeType) { // If we've been passed an XML node, it's a declaration
				...
				if(self.qtiElement.identifier) {
					self.identifier = self.qtiElement.identifier;
				}
				if(self.qtiElement.cardinality) {
					self.identifier = self.qtiElement.cardinality;
				}
				if(self.qtiElement.baseType) {
					self.identifier = self.qtiElement.baseType;
				}			
			} else {
	...
	QTI.Variable.prototype = {
		identifier: null,
		qtiElement: null, // The QTI declaration node
		cardinality: 'single',
		baseType: 'identifier',
		...
		setValue: function(value) {
			var self = this;
			if($.isArray(value)) {
				self.value = value.slice(); // copy the contents
			} else {
				self.value = [value];
			}
		},
		...
		containsValue: function(value) {
			var self = this;
			return ($.inArray(value, self.value) != -1);
		},
		...
		newInstance: function() {
			var self = this;
			var result = $.extend(true, { value: null }, self);
			result.setValue(self.getDefaultValue());
			return result;
		}
	};
	...
	$.widget("qti.assessmentItem", {
		...
		startSession: function() {
			...
			opts.responseProcessing.assessmentItem = self;
		...
		showFeedback: function() {
			...
			$(':qti-feedbackInline, :qti-feedbackBlock', self.element).each(function(i, o) {
				...
				var varsEqual = (self.options.responseProcessing.variables[variableName].containsValue(identifier));
			...
			$(':qti-modalFeedback', self.element).each(function(i, o) {
				console.log('processing modalFeedback');
				$(o).hide(); // hide it first
				...
				// Process as in http://wiki.cetis.ac.uk/AssessmentItem:Modal_Feedback
				var varsEqual = (self.options.responseProcessing.variables[variableName].value == identifier);
				var showHide = $(qtiElement).attr('showHide');
				if(showHide == 'show') {
					if(varsEqual) {
